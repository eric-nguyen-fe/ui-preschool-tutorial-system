...
const OtherCalendar: React.FC = () => {
  ...
  const [isButtonLink, setIsaButtonLink] = useState(false);
  ...
  useEffect(() => {
    const hi = userData?.departments.filter((department) => (department.label === departmentName));
  ...
  useEffect(() => {
    if (!isOpen) {
      setTimeout(() => {
        ...
        setIsChecked(false);
        setIsaButtonLink(false);
        setValue('');
      }, 500);
    }
    if (isUpdate && selectedEvent?.meet_lab) {
      setIsChecked(true);
      setValue(selectedEvent.meet_lab);
      setIsaButtonLink(true);
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    if (selectedEvent !== undefined) {
      ...
      setDetailName(selectedEvent._name || '');
    }
  }, [selectedEvent]);
  ...
  const { mutate: postTutorial } = useMutation(
    async (data: Event) => axiosInstance.post(`booking/add-booking/${123} `, data),
    {
      onSuccess: (data) => {
        dispatch(getOtherUserAction(Number(userId) || 0));
  ...
  const handleConfirm = async () => {
    ...
    if (isUpdate) {
      ...
      updatedEvent[index]._name = detailName;
      ...
      updatedEvent[index].meet_lab = value;
      ...
      const newEventGuest = {
        ...
        title: detail,
        department: {
          id_derp: Math.floor(Math.random() * (50000 - 10 + 1)) + 10,
  ...
  useEffect(() => {
    if (isChecked && value === '') {
      setValue('https://meet.remotedu.castis.io/CS0011');
    }
  }, [isChecked, value]);
            ...
            <div className="p-calendar" ref={getHeightOtherPage}>
              <div className="p-calendar_calendarOther">
                ...
                <PopupBooking
                  ...
                  onChangValueName={(value) => setDetailName(value.target.value)}
                  valueTest={detail}
                  valueName={detailName}
                  ...
                  textButtonLink="Go to meeting room"
                />
              </div>
              <div className={mapModifiers('p-calendar_departmentOther p-calendar_department', isShowSideBar && 'showbar')}>
                {/* <SwitchDivision
                  ...
                  division={division}
                /> */}
                <Typography modifiers={['goldenrod', '700', '32x42']}>SCHEDULE</Typography>
              </div>

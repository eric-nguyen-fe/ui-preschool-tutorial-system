...
import iconLogo from 'assets/icons/ic_user.svg';
...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className={`m-dropdown ${typeDropdown === 'phone_number' ? 'm-dropdown_phone_number' : ''}`} ref={dropdownRef} onClick={!isDisabled ? toggling : () => isDisabled}>
      {isAvatar ? <Image src={iconLogo} alt="avatar header" />
        : (
      ...
      <Typography
        content={name}
        type="p"
        modifiers={typeDropdown === 'phone_number' ? ['400', 'dimGray', 'uppercase', '14x21'] : ['700', 'goldenrod', 'uppercase']}
      />
      <div className={mapModifiers('m-dropdown_icon', isOpen && 'open_list')}>
        <Icon iconName={typeDropdown === 'phone_number' ? 'dropDown' : 'dropDownGold'} size={typeDropdown === 'phone_number' && '12x12' || '17x12'} isPointer />
      </div>
      {isOpen
        && (
              ...
              optionsChild.map((option, idx) => (
                <Link href={option.slug} key={`m-dropdown_list${idx.toString()}`}>
                  ...
                  >
                    ...
                    >
                      <Typography type="p" modifiers={['blackolive', 'capitalize', '400']} content={option.name} />
                    </div>
                  </li>
                </Link>
              ))) : (null)}

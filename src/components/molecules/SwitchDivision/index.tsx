  ...
  return (
    <div className="m-division" ref={dropdownRef} onClick={!isDisabled ? toggling : () => isDisabled}>
      <div className={mapModifiers('m-division_btn', isOpen && 'rarote')}>
        <Typography type="h2" content={optionDavision.length ? (division?.label || optionDavision[0].label) : username} isDivision modifiers={['goldenrod', 'capitalize', '16x24', '700']} />
          ...
          <ul className="m-division_list">
            {
              optionDavision.length ? (optionDavision.map((option, idx) => (
                ...
                >
                  <Typography modifiers={['blackolive', 'capitalize', '20x30', '700']} content={option.label} />
                </li>
